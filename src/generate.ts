...
if (allTypes.length > 0) {
  console.log(`📦 Generating ${allTypes.length} individual model files...`);
  ...
  allTypes.forEach(typeCode => {
    ...
    console.log(`  ✅ ${modelFileName}${validDependencies.length > 0 ? ' (with imports: ' + validDependencies.join(', ') + ')' : ''}`);
  });
  ...
  const indexContent = [
    ...
    ` * Generated by improved-generate.ts`,
    ` */`,
    ``,
  ...
  console.log(`✅ Generated ${generatedModelFiles.size} individual model files`);
  console.log(`✅ Models index file created: ${indexFile}`);
}
...
Object.entries(groupedByTag).forEach(([tag, methods]) => {
  ...
  methods.forEach(({ fnName, requestBody, responseBody, pathParams, queryParams }) => {
    ...
    console.log(`  📋 ${fnName}: ${reqType} → ${resType} (path: ${pathInfo}, query: ${queryInfo})`);
  ...
  console.log(`✅ Controller interface file created: ${interfaceFile}`);
});
...
Object.entries(groupedByTag).forEach(([tag, methods]) => {
  ...
  routeLines.push(` * Generated by improved-generate.ts`);
  routeLines.push(` */`);
  routeLines.push('');
  ...
  console.log(`  📍 Tag "${tag}" paths: ${allPaths.join(', ')} → common prefix: "${commonPrefix}"`);
  ...
  console.log(`✅ Route file created: ${routeFile}`);
  console.log(`  🛣️  Generated ${methods.length} route(s) with middleware: [${Array.from(usedMiddleware).join(', ')}]`);
});

console.log(`Files created in ${generatedDir}`);
console.log(`Generated ${globalSeenTypes.size} unique types across all endpoints`);
console.log(`Generated ${Object.keys(groupedByTag).length} controller interface files for tags: [${Object.keys(groupedByTag).join(', ')}]`);
console.log(`Generated ${Object.keys(groupedByTag).length} route files with middleware configuration`);
console.log(`All types are modularized in individual files in ${modelsDir} for easy reuse!`);

if (parameterTypes.size > 0) {
  console.log(`🔄 Generated ${parameterTypes.size} reusable parameter types:`);
  parameterTypes.forEach((_, typeName) => {
    console.log(`  ♻️  ${typeName}`);
  });
} else {
  console.log(`ℹ️  No reusable parameter types were created`);
}
